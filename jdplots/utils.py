...
def plot_atoms8(Phi,Ys,outname=None):
    ...
    print('power ratio',np.sum(power[idx_to_plot])/np.sum(power))
...
def compute_losses_eps(Phi,W,H,estC,Ys,eps_nmf,eps_c):
    ...
    Cs0 = compute_tlnmfloss_eps(V,V_hat,eps_nmf,eps_c)
    return Cs0,Ls0,Is0
...
def solve_NMF(Phi,Ys,niter,eps_c,W0,H0):
    regul = 0
