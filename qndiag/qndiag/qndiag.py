...
def qndiag(C, B0=None, weights=None, max_iter=1000, tol=1e-6,
           ...
           return_B_list=False, verbose=0, proj = 0):
    ...
    for t in range(max_iter):
        ...
        if g_norm < tol * np.sqrt(n_features):  # rescale by identity
            print('break with g_norm = ', g_norm)
        ...
        if success:
            ...
            B = new_B
            if proj == 1:
                # normalize each row of B to unit norm
                rownorm_B = np.linalg.norm(B,axis=1)
                #print('rownorm',rownorm_B)
                B = np.diag(1/rownorm_B) @ B
                D = transform_set(B, C)
            #elif proj == 2:
            #    # project B to a unitary matrix by computing SVD
            #    B = unitary_projection(B)
            #    D = transform_set(B, C)
